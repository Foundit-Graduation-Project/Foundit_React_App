...
export function ChatSidebar({ chats, activeChatId, onSelectChat, isHiddenOnMobile }) {
  return (
    <aside className={`w-full md:w-[320px] lg:w-95 border-r border-slate-200 flex flex-col bg-white shrink-0 transition-transform ${isHiddenOnMobile ? 'hidden md:flex' : 'flex'}`}>
        ...
        {chats.map((chat) => (
          ...
          >
            <div className="relative shrink-0">
              <div className={`w-12 h-12 rounded-lg ${chat.productColor} flex items-center justify-center text-xs font-bold text-slate-400 border border-slate-100`}>
                Img
              </div>
              {chat.online && (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                {chat.unread && <div className="w-2 h-2 rounded-full bg-blue-600 shrink-0"></div>}
              </div>
            </div>
          </div>
        ))}
